...
def test_winapp(tmp_path: Path):
    winapp = parse_pe("PE/PE64_x86-64_binary_WinApp.exe")
...
def test_pgo(tmp_path: Path):
    ...
    new = lief.PE.parse(output)
    assert new is not None
    new_lc = new.load_configuration
    assert new_lc is not None
    assert new_lc.security_cookie == 0xDEADC0DE
...
def test_seh_functions():
    ...
    functions = lconfig.seh_functions
...
def test_guard_cf_functions():
    ...
    functions = lconfig.guard_cf_functions
...
def test_dynamic_relocations_1():
    ...
    dyn_relocs = lc.dynamic_relocations
    ...
    assert (
        fixups.relocations[0].import_type
        == lief.PE.DynamicFixupARM64Kernel.IMPORT_TYPE.STATIC
    )
...
def test_dynamic_relocations_3():
    ...
    assert (
        fixups.relocations[272].type == lief.PE.DynamicFixupARM64X.FIXUP_TYPE.ZEROFILL
    )
...
def test_enclave_config():
    ...
    lc = pe.load_configuration
    assert lc is not None
    assert lc.enclave_configuration_ptr == 0x18000A910
...
def test_volatile_metadata():
    ...
    lc = pe.load_configuration
    assert lc is not None
    assert lc.volatile_metadata_pointer == 0x180383894
    metadata = lc.volatile_metadata
...
@pytest.mark.private
def test_issue_1273():
    pe = parse_pe("private/PE/issue_1273.pe")
    assert pe is not None
