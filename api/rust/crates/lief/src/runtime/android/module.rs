...
pub struct Module {
    ptr: cxx::UniquePtr<ffi::runtime_android_Module>,
}

impl FromFFI<ffi::runtime_android_Module> for Module {
    fn from_ffi(ptr: cxx::UniquePtr<ffi::runtime_android_Module>) -> Self {
        Self { ptr }
    }
}
...
impl Module {
    ...
    pub fn from_handle(handle: *const c_void) -> Option<Module> {
        into_optional(ffi::runtime_android_Module::from_handle(handle as u64))
    }
...
pub fn dlopen(name: &str) -> Option<Module> {
    cxx::let_cxx_string!(s = name);
    into_optional(ffi::runtime_android_dlopen(&s))
}
